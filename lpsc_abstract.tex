...
\titlearea{Using \LaTeX\ to write an LPSC Abstract.}{Ross A. Beyer, NASA Ames Research Center, MS 245-3, Moffett Field, CA, USA (rbeyer@arc.nasa.gov)}

% Here is an example with more than one author from more than one place:
%
% \titlearea{Using \LaTeX\ to write an LPSC Abstract.}{Ross A. Beyer$^{1}$, John Q. Public$^{a}$, and Jane Doe$^{\dag}$, $^{1}$NASA Ames Research Center, MS 245-3, Moffett Field, CA, USA (rbeyer@arc.nasa.gov), $^{a}$123 Sesame Street, New York NY, USA, $^{\dag}$890 Somewhere, ID, USA}
